...
function solution(N, P, Q) {
  ...
  for (let k = 0; k < M; k++) semiRanges[k] = Q[k] < 4 ? 0 : semisCount[Q[k]] - semisCount[P[k] - 1];
...
function semis(N) {
  ...
  for (let i = 4; i < rsc.length; i++) {
    if (sieve[i] != 0 && sieve[parseInt(i / sieve[i])] == 0) sc++;
    rsc[i] = sc;
  }
...
function siever(N) {
  ...
  for (let i = 2; i <= Math.sqrt(N); i++) {
    if (sieve[i] == 0) {
      for (let j = i * i; j <= N; j += i) if (sieve[j] == 0) sieve[j] = i;
    }
  }
